...
function wrapper(my) {
    ...
    return function(req, start) {
        ...
        function finish() {
            return cleanup(), void 0 !== first[1] ? (story._bytesDispatched += soc._bytesDispatched, 
            story.bytesRead += soc.bytesRead, req.transferRate = oi(story, first, !0), req.transferRate) : void 0;
        }
        function cleanup() {
            soc.removeListener("close", close);
        }
        if (null === req || "object" != typeof req) throw new TypeError("req required");
        if (null === start || Array.isArray(start) === !1) throw new TypeError("start required");
        var res = req.res;
        void 0 === first[1] && (first = start);
        var t = res.finished || res.socket && res.socket.writable === !1;
        if (t === !0) return req.transferRate = oi(req.socket, first), req.transferRate;
        var soc = req.socket;
